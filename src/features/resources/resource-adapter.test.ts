...
describe("Markets SDK resource adapter", () => {
  it("passes the canonical collection through unchanged and serializes one list query contract", async () => {
    vi.stubEnv("VITE_API_BASE_URL", "https://markets.example.com");
    const canonicalResult = {
      items: [{ uid: "category-1", display_name: "Rates" }],
      pageInfo: {
        pageIndex: 1,
        pageSize: 25,
        totalItems: 51,
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse(canonicalResult));
    ...
    expect(url.searchParams.has("response_format")).toBe(false);
  });

  it("discovers, preflights, and executes backend-owned bulk actions through SDK contracts", async () => {
    vi.stubEnv("VITE_API_BASE_URL", "https://markets.example.com");
    const fetchMock = vi.spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(jsonResponse(resourceDiscovery({
        id: "asset-categories",
    ...
    const application = createMarketsResourceApplication(assetCategoriesDefinition);
    const controller = new AbortController();
    const discovery = await application.adapter.discover!({
      search: "rates",
      filters: { source: "official" },
    }, { signal: controller.signal });
    const [action] = discovery.bulk_actions;
    const input = {
    ...
    expect(discoveryUrl.pathname).toBe("/api/v1/asset-category/discovery/");
  ...
  it("normalizes blocked preflight results without discarding backend evidence", async () => {
    ...
    const application = createMarketsResourceApplication(assetCategoriesDefinition);
    const discovery = await application.adapter.discover!({ filters: {} });
    const [action] = discovery.bulk_actions;
  ...
  it("rejects invalid backend bulk-action discovery instead of trusting unsafe endpoints", async () => {
    vi.stubEnv("VITE_API_BASE_URL", "https://markets.example.com");
    vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse(resourceDiscovery({
      id: "asset-categories",
      label: "Asset Categories",
      itemLabel: "asset categories",
      columnId: "display-name",
    ...
    await expect(application.adapter.discover!({ filters: {} })).rejects.toThrow(
...
function resourceDiscovery(input: {
  ...
  return {
    ...
    list: {
      controls: {
        search: { placeholder: `Search ${input.itemLabel}`, fields: [input.columnId] },
        filters: [],
        ordering: [],
      },
      columns: [{
