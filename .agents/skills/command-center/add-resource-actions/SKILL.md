...
- Obtain selection and bulk action availability, endpoints, selection modes, options, confirmation,
  tone, and optional preflight from the normalized resource adapter when the backend owns them.
...
4. Preserve `explicit` and `all_matching` bulk selection semantics exactly.
5. Pass advertised options and canonical selection unchanged to preflight and execution.
6. Run advertised preflight when confirmation opens and whenever selection or options change.
7. Keep confirmation disabled until the current preflight allows execution.
8. Preserve pending, error, blocked, retry, refresh, and selection cleanup owned by
   `ResourceListPage`.
...
semantics. Keep backend implementation outside this skill.
...
cancellation, retry, transport failure, successful refresh, and selection cleanup.
